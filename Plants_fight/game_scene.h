...
public:
	...
	void on_enter() override {
		...
		timer_winner_slide_out.set_wait_time(2000);
	...
	void on_update(int delta) override {
		...
		if (player_1->get_hp() <= 0 || player_2->get_hp() <= 0) {
			if (!is_game_over) {
				mciSendString(_T("stop bgm_scene"), NULL, 0, NULL);
		...
		if (is_game_over) {
			...
			else timer_winner_slide_out.on_update(delta);
		}
	...
	void on_draw(const Camera& camera) override {
		...
		if (is_debug) {
			...
			outtextxy(15, 15, _T("已开启调试模式，再次摁‘Q’键退出"));
		}
	}
...
private:
	...
	const float speed_winner_bar = 2.0f;
	const float speed_winner_text = 1.0f;
